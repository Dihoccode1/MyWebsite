...
/*
  ...
  - TÍCH HỢP KHO: trừ kho NEW→CONFIRMED/DELIVERED, hoàn kho CONFIRMED→CANCELED
...
(function () {
  ...
  const KEYS = {
    ...
    STOCK   : 'admin.stock',         // phiếu nhập/xuất
    CAT_BMP : 'catalog.bump',
    STK_BMP : 'stock.bump',
  };

  const USER_KEYS_IMPORT = ['sv_orders_flat', 'sv_orders_v1', 'sv_orders', 'orders'];
  ...
  // Hiển thị tiền tệ thống nhất với UI cũ (không thêm ₫ để không phá layout cũ)
  ...
  function normalize(list) {
    return (Array.isArray(list) ? list : []).map((o, i) => {
      ...
      const total = Number(o.total) || items.reduce((s, it) => s + (Number(it.price)||0)*(Number(it.qty)||0), 0);
      ...
      const code = o.code || o.order_code || (typeof o.id === 'string' ? o.id : `OD-${created.slice(0,10).replace(/-/g,'')}-${String(i+1).padStart(3,'0')}`);
  ...
  (function importOnce() {
    ...
    for (const k of USER_KEYS_IMPORT) {
      try {
        ...
        // k = sv_orders_v1 có thể là object {email: []}
        if (k === 'sv_orders_v1') {
  ...
  function loadAdminOrdersMerged() {
    ...
    for (const u of (Array.isArray(userFlat)?userFlat:[])) {
      ...
      if (map.has(id)) {
        const a = map.get(id);
        // copy các field chính nếu u có dữ liệu
    ...
    adminList.sort((x,y)=> new Date(y.created_at||0) - new Date(x.created_at||0));
  ...
  function writeOrderEverywhere(order) {
    ...
    for (const email of Object.keys(userStore||{})) {
      ...
      const uidx = arr.findIndex(o => String(o.id) === id || String(o.code) === order.code);
      if (uidx >= 0) {
        arr[uidx] = order;
        userStore[email] = arr;
        touched = true;
    ...
    // nếu không tìm thấy ở email khác, nhưng order.email có thì upsert vào email đó
    if (!touched && order.email) {
      const arr2 = Array.isArray(userStore[order.email]) ? userStore[order.email] : [];
      const u2 = arr2.findIndex(o => String(o.id) === id || String(o.code) === order.code);
      if (u2 >= 0) arr2[u2] = order; else arr2.push(order);
    ...
    try { localStorage.setItem(KEYS.PING1, String(Date.now())); } catch {}
  }

  /* ================== KHO: products & stock ================== */
  function prodsList() { return LS.get(KEYS.PROD, []); }
  function prodsSave(arr) { LS.set(KEYS.PROD, arr||[]); try{ localStorage.setItem(KEYS.CAT_BMP, String(Date.now())); }catch{} }
  function txList() { return LS.get(KEYS.STOCK, []); }
  function txSave(arr) { LS.set(KEYS.STOCK, arr||[]); try{ localStorage.setItem(KEYS.STK_BMP, String(Date.now())); }catch{} }

  function findProdForItem(pList, it) {
    // id hoặc code (khớp hoa-thường)
    const byId = pList.find(p => String(p.id) === String(it.id));
    if (byId) return byId;
  ...
  function applyStockForOrder(order, sign) {
    ...
    order.items.forEach(it => {
      ...
      if (!p) return;
      const q = Number(it.qty || 0) * (sign || -1);
      const newQty = Math.max(0, Number(p.qty || 0) + q); // tránh âm
      p.qty = newQty;

      txs.push({
        id        : 'STK_' + Math.random().toString(36).slice(2,8) + Date.now().toString(36).slice(-4),
        productId : p.id,
  ...
  // Chỉ trừ/hoàn đúng trường hợp chuyển trạng thái
  function handleStockTransition(prev, next, order) {
    if (!order) return;
    const P = String(prev || 'new');
    const N = String(next);
    if (P === 'new' && (N === 'confirmed' || N === 'delivered')) applyStockForOrder(order, -1);
  ...
  function updateStatus(id, status) {
    ...
    const next = (String(status).toLowerCase() === 'cancelled') ? 'canceled' : String(status).toLowerCase();

    // cập nhật đối tượng
    const o = { ...list[i], status: next, updatedAt: new Date().toISOString() };
    if (next === 'delivered') o.deliveredAt = new Date().toISOString();
    if (next === 'canceled')  o.canceledAt  = new Date().toISOString();

    // Ghi lại ngay để UI phản hồi
    list[i] = o;
    LS.set(KEYS.ADMIN, list);

    // TÍCH HỢP KHO theo chuyển trạng thái
    try { handleStockTransition(prev, next, o); } catch (e) { console.warn('stock transition failed', e); }
  ...
  function render() {
    ...
    tb.innerHTML = state.filtered.map(o => {
      ...
      const addr = [o?.shipping?.address,o?.shipping?.ward,o?.shipping?.district,o?.shipping?.city]
        .filter(Boolean).join(', ');
  ...
  /* ================== Init ================== */
  (function init() {
    // Nếu chưa có FLAT thì build từ USER
    if (!Array.isArray(LS.get(KEYS.FLAT))) flattenUserOrders();

    // Mốc thời gian default: 7 ngày gần nhất
    const now = new Date();
