...
<head>
  ...
  <style>
    /* Breadcrumbs */
    .breadcrumbs{padding:15px 0;font-size:13px;color:#777} .breadcrumbs a{color:#333}

    /* View controls */
    .view-controls{display:flex;justify-content:space-between;align-items:center;padding:15px 0}
    .filter-select,.form-control{padding:8px 12px;border:1px solid #ccc;font-size:13px}
...
<body>
  ...
  <div class="container">
    ...
    <!-- Form tìm kiếm / lọc -->
    <form id="saleSearchForm" class="form-inline mb-2">
  ...
  <!-- Trang KM tự render (không phụ thuộc ui.js chung) -->
  <script>
    (function () {
      // === Helpers giống store.js ===
      ...
      // BỘ LỌC: chỉ giữ SP khuyến mãi
      function getSaleBase(){
        const all = (window.SVStore?.getAllProducts?.() || window.SV_PRODUCT_SEED || []);
        return all.filter(p => {
          const hasBadge = normalizeBadge(p) === 'sale';
          const hasPriceCut = toNumber(p.original_price) > toNumber(p.price);
          return hasBadge || hasPriceCut;
        });
      }
      ...
      const form = document.getElementById('saleSearchForm');
      ...
      function cardHTML(p){
        const badge = normalizeBadge(p);
        const badgeHTML =
          badge === 'sale' ? '<span class="product-badge badge-sale">Sale</span>' :
